...
function OTP_Dismiss_Pointers() {
  jQuery('.otp-skip-all-tutorial-pop-ups').on('click', function() {
    jQuery.post( ajaxurl, { action: 'otp-dismiss-wp-pointers'});
...
jQuery(document).ready(function($) {
  jQuery('body').on('DOMNodeInserted', function(event) {
    if (jQuery(event.target).hasClass('ewd-otp-pointer-count')) {OTP_Dismiss_Pointers();}
  });
});