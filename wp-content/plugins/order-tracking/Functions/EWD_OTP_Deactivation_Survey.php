<?php
add_action('current_screen', 'EWD_OTP_Deactivation_Survey');
function EWD_OTP_Deactivation_Survey() {
	if (in_array(get_current_screen()->id, array( 'plugins', 'plugins-network' ), true)) {
		add_action('admin_enqueue_scripts', 'EWD_OTP_Enqueue_Deactivation_Scripts');
		add_action( 'admin_footer', 'EWD_OTP_Deactivation_Survey_HTML'); 
	}
}

function EWD_OTP_Enqueue_Deactivation_Scripts() {
	wp_enqueue_style('ewd-otp-deactivation-css', EWD_OTP_CD_PLUGIN_URL . 'css/ewd-otp-plugin-deactivation.css');
	wp_enqueue_script('ewd-otp-deactivation-js', EWD_OTP_CD_PLUGIN_URL . 'js/ewd-otp-plugin-deactivation.js', array('jquery'));

	wp_localize_script('ewd-otp-deactivation-js', 'ewd_otp_deactivation_data', array('site_url' => site_url()));
}

function EWD_OTP_Deactivation_Survey_HTML() {
	$Install_Time = get_option("EWD_OTP_Install_Time");
	...
	?>
	<div class="ewd-otp-deactivate-survey-modal" id="ewd-otp-deactivate-survey-ultimate-faqs">
		<div class="ewd-otp-deactivate-survey-wrap">
			<form class="ewd-otp-deactivate-survey" method="post" data-installtime="<?php echo $Install_Time; ?>">
				...
				<span class="ewd-otp-deactivate-survey-desc"><?php echo __('If you have a moment, please share why you are deactivating Status Tracking:', 'order-tracking' ); ?></span>
				<div class="ewd-otp-deactivate-survey-options">
					<?php foreach ( $options as $id => $option ) : ?>
						<div class="ewd-otp-deactivate-survey-option">
							<label for="ewd-otp-deactivate-survey-option-ultimate-faqs-<?php echo $id; ?>" class="ewd-otp-deactivate-survey-option-label">
								<input id="ewd-otp-deactivate-survey-option-ultimate-faqs-<?php echo $id; ?>" class="ewd-otp-deactivate-survey-option-input" type="radio" name="code" value="<?php echo $id; ?>" />
